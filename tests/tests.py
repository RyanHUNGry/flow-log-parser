...
class TestParserCLI(unittest.TestCase):
    ...
    def test_set_schema_all(self):
        ...
        self.assertEqual(self.cli.schema, list(constants.FLOW_LOG_SCHEMA_ALL))
    ...
    def test_load_and_search_commands_from_file(self):
        try:
            ...
            # also test optional output file for search_src
            self.stdout.truncate(0); self.stdout.seek(0)
            self.cli.onecmd("search_src 10.0.0.159 output.txt")
            self.assertTrue(os.path.exists("output.txt"))
            with open("output.txt", "r") as f:
                data = json.load(f)
                data[0]["srcaddr"] = "10.0.0.159"
            ...
            # also test optional output file for search_dst
            self.stdout.truncate(0); self.stdout.seek(0)
            self.cli.onecmd("search_dst 10.0.0.23 output.txt")
            self.assertTrue(os.path.exists("output.txt"))
            with open("output.txt", "r") as f:
                data = json.load(f)
                data[0]["dstaddr"] = "10.0.0.23"
            os.remove("output.txt")
            out = self._out()
            self.assertIn("Total results: 3384", out)
            ...
            # search by src+dst
            self.stdout.truncate(0); self.stdout.seek(0)
            self.cli.onecmd("search_src_dst 10.0.3.104 10.0.1.194 output.txt")
            self.assertTrue(os.path.exists("output.txt"))
            with open("output.txt", "r") as f:
                data = json.load(f)
                data[0]["srcaddr"] = "10.0.3.104"
                data[0]["dstaddr"] = "10.0.1.194"
            os.remove("output.txt")
            out = self._out()
    ...
    def test_load_and_search_commands(self):
        # create temp file with three sample flowlog lines
    ...
    def test_load_and_search_commands_custom_schema(self):
        # create temp file with three sample flowlog lines
        fd, path = tempfile.mkstemp(prefix="flowlogs_", suffix=".txt")
        os.close(fd)
        try:
            with open(path, "w") as f:
                # interface-id srcaddr srcport dstaddr dstport protocol packets bytes start end action log-status
                SAMPLE_LINE_1 = "eni-1 10.0.0.1 1000 10.0.0.2 2000 6 1 100 1600000000 1600000001 ACCEPT OK\n"
                SAMPLE_LINE_2 = "eni-1 10.0.0.3 1500 10.0.0.2 2500 6 2 200 1600000100 1600000110 REJECT OK\n"
                SAMPLE_LINE_3 = "eni-ab12cd34ef5678901 - - - - - - - 1732930480 1732930522 - SKIPDATA\n"
                f.write(SAMPLE_LINE_1)
                f.write(SAMPLE_LINE_2)
                f.write(SAMPLE_LINE_3)
            ...
            # load file
            self.stdout.truncate(0); self.stdout.seek(0)
            self.cli.onecmd("set_schema interface-id srcaddr srcport dstaddr dstport protocol packets bytes start end action log-status")
            self.cli.onecmd(f"load {path}")
            out = self._out()
            self.assertIn(f"Loading flow logs from {path}", out)
            self.assertTrue(hasattr(self.cli, "parser"))
            self.assertEqual(self.cli.schema, ["interface-id", "srcaddr", "srcport", "dstaddr", "dstport", "protocol", "packets", "bytes", "start", "end", "action", "log-status"])
